...
def plotly_table(dataframe):
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells=dict(
            values=[[f"<b>{i}</b>" for i in dataframe.index]] + [dataframe[col].tolist() for col in dataframe.columns],
            fill_color=[[rowEvenColor if i % 2 == 0 else rowOddColor for i in range(len(dataframe))]] * (len(dataframe.columns) + 1),
            align='left',
            line_color='white',
...
def filter_data(dataframe, num_period):
    if num_period == '1mo':
        ...
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(years=-10)
    elif num_period == 'ytd':
        date = datetime.datetime(dataframe.index[-1].year, 1, 1)
    else:
        date = dataframe.index[0]  # fallback default
...
def close_chart(dataframe, num_period = False):
   ...
   #  fig.update_xaxes(rangeslider_visible= True) for the second small graph
...
def RSI(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(
        ...
        x = dataframe['Date'],
        y= [70]*len(dataframe),  marker_color = 'red', line = dict(width=2, color ='red', dash = 'dash'),
...
def MACD(dataframe, num_period):
    ...
    fig.update_layout(
        height=200,
        plot_bgcolor='white',
        paper_bgcolor='#e1efff',  # Fixed hex color
...
def Moving_average_forecast(forecast):
    ...
    # fig.add_trace(go.Scatter(x=forecast.index[-30:], y=forecast['Close'].iloc[-30:], 
                           #  mode = 'lines', name = 'Close Price', line = dict(width=2, color = 'black')))     for past stock data
    ...
    # fig.update_xaxes(rangeslider_visible=True)  # Remove it for one graph
    fig.update_layout(height = 500, margin = dict(l=0, r=20, t=20, b=0), plot_bgcolor = 'white', paper_bgcolor= '#e1efff', legend = dict(
        yanchor ="top",
        xanchor = "right"
    ))
