...
def get_rolling_mean(close_price):
    rolling_price = close_price.rolling(window=7).mean().dropna()   # 7 Days 
    return rolling_price     # So that fluctuation come less and help in predicting upcoming days stock 

def get_differencing_order(close_price):
    p_value = stationary_check(close_price)
    d = 0    #v d for differencing
    while True:
        if p_value > 0.05:
            d += 1
